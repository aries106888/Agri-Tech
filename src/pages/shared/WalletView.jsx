...
const WalletView = ({ role = 'farmer' }) => {
  ...
  const handleWithdraw = async (e) => {
    ...
    try {
      ...
      setTransactions(prev => [newTx, ...prev]);
      setActionSuccess('Withdrawal initiated! Funds will arrive shortly.');
      setTimeout(() => setModal(null), 1500);
    } catch {
      setBalance(prev => prev - withdrawAmt);
      const newTx = {
      ...
      setTransactions(prev => [newTx, ...prev]);
      setActionSuccess('Withdrawal initiated!');
      setTimeout(() => setModal(null), 1200);
    } finally {
      setActionLoading(false);
    }
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl animate-slide-up border border-ag-border">
            ...
            {modal === 'withdraw' && (
              <form onSubmit={handleWithdraw} className="flex flex-col gap-4">
                ...
                >
                  {actionLoading ? 'Processing withdrawal...' : `Withdraw KSh ${amount ? parseFloat(amount).toLocaleString() : '0'}`}
                </button>
              </form>
            )}
