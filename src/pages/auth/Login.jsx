...
import { Mail, Lock, AlertCircle, Leaf, CheckCircle, Shield, Tractor, Briefcase, Truck } from 'lucide-react';
...
const Login = () => {
  ...
  const [selectedRole, setRole] = useState('farmer'); // for non-admin login
  ...
  useEffect(() => {
    if (user && role) {
      // If we're on the admin portal but the logged-in role isn't admin, stay
      // and let handleSubmit's explicit check show the error + signOut.
  ...
  }, [user, role, navigate, location, isAdminRoute]);
