...
if (isset($request[0]) && $request[0] === 'auth') {
    ...
    if (isset($request[1]) && $request[1] === 'register' && $method === 'POST') {
        ...
        if (!in_array(strtolower($body['role']), $validRoles)) {
        ...
        respond([
            ...
            "user"    => [
                ...
                "email" => $body['email'],
                "phone" => $body['phone'],
                "role"  => strtolower($body['role']),
            ],
            "token"   => base64_encode($body['email'] . ':' . strtolower($body['role']) . ':' . time())
        ], 201);
    }
    ...
    if (isset($request[1]) && $request[1] === 'login' && $method === 'POST') {
        ...
        $role     = strtolower($body['role'] ?? 'buyer');
...
if (isset($request[0]) && $request[0] === 'orders') {
    ...
    if ($method === 'POST') {
        ...
        respond([
            ...
            "order_id" => strtoupper(substr(md5((string)time()), 0, 8)),
            "status"   => "pending",
            "items"    => $body['items'],
...
if (isset($request[0]) && $request[0] === 'logistics') {
    ...
    if ($method === 'POST') {
        ...
        respond([
            ...
            "request_id"       => strtoupper(substr(md5((string)time()), 0, 6)),
            "driver"           => $driver,
            "pickup"           => $body['pickup'],
    ...
    if ($method === 'GET') {
        respond(array_values(array_filter($drivers, fn($d) => $d['status'] === 'available')));
    }
}
...
// ─── 404 ──────────────────────────────────────────────────────────────────────
respond(["error" => "Route not found. Visit /api to see all available routes."], 404);
?>
